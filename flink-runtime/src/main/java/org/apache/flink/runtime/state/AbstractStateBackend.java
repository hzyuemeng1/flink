...
public abstract class AbstractStateBackend implements java.io.Serializable {
	 ...
	 *
	 * 首先：
	 *     stateBackend，每个AbstractStreamOperator都有一个，什么时候创建的可以跟上去。
	 *
	 *     stateDescriptor描述一个state，stateDescriptor.bind 创建一个具体的state。
	 *     然后使用这个具体的state
	 *
	 *     ValueState就是一个键值对，有一个currentKey
	 *     接着是最重要的，currentKey的来源是 	streamOperator.setKeyContextElement1(record);//通过select
											streamOperator.processElement(record);
	 *
	 *      这里每创建的kvstate都是基于这个currentkey
	 *
	 *
	 *
	 *
	 *
	 *   关于namespace  ：参考org/apache/flink/streaming/runtime/operators/windowing/WindowOperator.java:263
	 *                  一看就明白
	 *
	 *
	 *
