...
public class IterativeStream<T> extends SingleOutputStreamOperator<T> {
	...
	public DataStream<T> closeWith(DataStream<T> feedbackStream) {
		...
		//getTransformation返回的是第45行的FeedbackTransformation，
		// feedbackStream.getTransformation()返回的比如是filter的OneInputTransformation
	 ...
	 *
	 *
	 * 另外，IterativeStream通过调用withFeedbackType方法还可以改变或者重新指定迭代反馈流的类型，
	 * 从而形成一个跟最初的输入流组合而成的连接迭代流（ConnectedIterativeStreams），这一点也是批处理中的迭代所不具备的
	 *
	 * @param feedbackType
