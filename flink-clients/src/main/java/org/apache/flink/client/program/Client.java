...
public class Client {
	...
	public JobExecutionResult runBlocking(JobGraph jobGraph, ClassLoader classLoader) throws ProgramInvocationException {
		...
		try {
			/**
			 * LeaderRetrievalService在Flink中提供查找主节点的服务。它会根据Flink的配置信息
			 * （主要是recovery.mode来判断基于哪种恢复机制来创建该服务。当前有两种模式：一种是Standalone的独立运行模式；
			 * 另一种是基于Zookeeper的高可用模式）。Flink提供了一个称之为LeaderRetrievalListener的回调接口来获得主节点的信息
			 */
