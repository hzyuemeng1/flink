...
public class StreamGraph extends StreamingPlan {
	...
	private void addEdgeInternal(Integer upStreamVertexID,
		...
		if (virtualSelectNodes.containsKey(upStreamVertexID)) {
			...
			// select上游的节点id，  select的上游是split
			upStreamVertexID = virtualSelectNodes.get(virtualId).f0;
			if (outputNames.isEmpty()) { //第一次都为空
			 ...
			 * 这里非常注意：
			 *
			 * 像map、filter操作，如果不设置并行度，他们的输入直接会交给下一个确定的节点。
			 * 就是ForwardPartitioner,这些边其实在生成JobGraph不会有特别的处理，因为isChainable会把他们链成内部节点
			 *
			 * 但是org/apache/flink/streaming/api/graph/StreamingJobGraphGenerator.java:446
			 * 比如说Union操作，在此时也会设置为默认的ForwardPartitioner，Union之后的节点不会被连成内部节点，
			 * 所以再connect方法为那些"顶点"创建中间结果集的时候：
			 * 重点：connect创建的是headVertex的中间结果集，这个结果集会带上分区类型即istributionPattern.POINTWISE。
			 *
			 * 在生成执行计划的时候，，，，connectPointwise方法
			 *
			 *
			 *
			 */
