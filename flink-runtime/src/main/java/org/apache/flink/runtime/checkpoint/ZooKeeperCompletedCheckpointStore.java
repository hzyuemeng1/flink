...
public class ZooKeeperCompletedCheckpointStore implements CompletedCheckpointStore {
	...
	public void recover() throws Exception {
		...
		//恢复的过程首先是从Zookeeper获取所有的检查点，这里为了规避并发修改带来的失败，采用了循环重试的机制
        ...
        //在恢复时，将从Zookeeper中读取最新的检查点，如果检查点超过一个，仅仅最新的那个检查点有效，旧的都会被丢弃。
		// 如果存在着网络分区，多个JobManager的实例并发对相同的程序实行检查点，那么选择任意一个验证通过的已完成的检查点都是没有问题的。
