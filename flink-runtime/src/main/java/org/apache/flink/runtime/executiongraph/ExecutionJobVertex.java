...
public class ExecutionJobVertex implements Serializable {
	...
	public void connectToPredecessors(Map<IntermediateDataSetID, IntermediateResult> intermediateDataSets) throws JobException {
		...
		for (int num = 0; num < inputs.size(); num++) {
			...
			//ExecutionJobVertex添加上了JobVertex的输出
			this.inputs.add(ires);

			//现在是消费中间结果，仅仅是简单将numConsumers+1，将IntermediateResultPartition的消费者consumers+1
	...
	public void scheduleAll(Scheduler scheduler, boolean queued) throws NoResourceAvailableException {
		...
		if (inputSplitsPerSubtask != null) { //应该是dataSet相关的
			//拿到所有注册到jobManager的taskManager  key为hostName
