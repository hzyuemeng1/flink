...
public abstract class MemorySegment {
	 ...
	 * http://www.jb51.net/article/48318.htm  参考这个地址
	 * */
	@SuppressWarnings("restriction")
	...
	protected long address;//字节数组对应的相对地址（若heapMemory为BYTE_ARRAY_BASE_OFFSET，即可能为off-heap内存的绝对地址)

	/** The address one byte after the last addressable byte.
	 *  This is address + size while the segment is not disposed */
	protected final long addressLimit;// 标识地址结束位置（address+size）
	 ...
	 *
	 * UNSAFE.getInt(heapMemory, pos)的特点:
	 *
	 *
	 * （1）如果对象o不为null，并且后面的地址或者位置是相对位置，那么会直接对当前对象（比如数组）的相对位置进行操作，
	 * 既然这里对象不为null，那么这种情况自然满足on-heap的场景；
	  （2）如果对象o为null，并且后面的地址是某个内存块的绝对地址，那么这些方法的调用也相当于对该内存块进行操作。
