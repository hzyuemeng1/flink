...
public class SingleInputGate implements InputGate {
	...
	/**
	 * 有一种占位目的的UnknowInputChannel不响应该方法，因为它最终会被确定为是LocalInputChannel还是
	 * RemoteInputChannel，确定的时机通常是JobManager通知器其可消费，TaskManager调用当前SingleInputGate
	 * 的updateInputChannel方法，确定UnknowInputChannel会转变的具体的通道类型后再调用requestSubpartition方法。
	 *
	 *
	 *
	 * 由于requestPartitions只是起到触发其内部的InputChannel去请求的作用，这个调用可能并不会阻塞等待远程数据被返回。
	 * 因为不同的InputChannel其请求的机制并不相同，RemoteChannel就是利用Netty异步请求的。所以SingleInputGate采用
	 * 阻塞等待以及事件回调的方式来等待InputChannel上的数据可用。具体而言，它在while代码块中循环阻塞等待有可获取数据的InputChannel。
	 * 而可用的InputChannel则由它们自己通过回调
	 * SingleInputGate的onAvailableBuffer添加到阻塞队列inputChannelsWithData中来。
	...
	public BufferOrEvent getNextBufferOrEvent() throws IOException, InterruptedException {
		...
		while (currentChannel == null) {
			...
			/**
			 * 这个inputChannel的来源org/apache/flink/runtime/io/network/netty/PartitionRequestClientHandler.java:285
			 */
	...
	/**
	 *
	 * InputGate最关键的方法自然是获取生产者所生产的缓冲区，
	 * 提供该功能的方法为getNextBufferOrEvent
	 *
