...
public class ResultPartition implements BufferPoolOwner {
	 ...
	 * 	 BLOCKING：持久化、非管道、无反压；
	 ...
	 其中管道属性会对消费端任务的消费行为产生很大的影响。如果是管道型的，那么在结果分区接收到第一个Buffer时，消费者任务就可以进行准备消费
	 （如果还没有部署则会先部署），而如果非管道型，那么消费者任务将等到生产端任务生产完数据之后才会着手进行消费。
	 *
	 ...
	 *
	 * 一个ResultPartition有多少个ResultSubPartition，是在构建ResultPartition就确定了的。
	 * 当生产端任务调用记录写入器写入一个记录时，该记录先被序
	 * 列化器序列化并放入Buffer中，然后通过ResultPartitionWriter加入到ResultPartition，具体被加入哪个子分区中取决于ChannelSelector
	 *
	...
	public void add(Buffer buffer, int subpartitionIndex) throws IOException {
		...
		try {
			...
			if (success) {
				/**
				 * 在notifyPipelinedConsumers方法中，会通过分区可消费通知器（ResultPartitionConsumableNotifier）间接通知消费者任务
				 * （经过JobManager转发通知），它会携带两个信息：

						 JobID
						 ResultPartitionID
				 *
				 * ResultPartition有一个标识变量hasNotifiedPipelinedConsumers，用来表示当前是否已通知过消费者，在notifyPipelinedConsumers中，
				 * 一旦通知过，该标识将会被设置为true，所以该通知只会发生在第一个被成功加入的Buffer之后，后续便不再通知。
				 *
				 *
				 * ResultPartitionConsumableNotifier当前只有一个实现JobManagerResultPartitionConsumableNotifier
				 * （位于NetworkEnvironment中的一个静态内部类），它会通过Actor网关向JobManager发送一条请求消息（ask模式，需要应答）。

				 这是针对管道模式的ResultPartition而言的，而针对阻塞模式的ResultPartition的通知时机却需要等到数据生产完成之后
				 （ResultPartition的finish方法被调用），任务会向JobManager报告其状态变更为FINISHED。
				 JobGraph根据执行图（ExecutionGraph）找到完成任务对应的IntermediateResultPartition的消费者任务并调度它们进行消费
				 所以请看finish方法
	...
	public void finish() throws IOException {
		...
		try {
			...
			for (ResultSubpartition subpartition : subpartitions) {
				synchronized (subpartition) {
					subpartition.finish();//会发送EndOfPartitionEvent.INSTANCE
				}
			}
	 ...
	 *
	 * 当每个子分区中的缓冲区数据被消费完后，它们会通知ResultPartition。因为一个ResultPartition包含若干个
	 * ResultSubPartition，那么ResultPartition如何判断所有的ResultSubPartition都被消费完了呢？
	 * 它基于原子计数器（AtomicInteger），每个ResultSubPartition被消费完成之后都会回调ResultPartition
	 * 的实例方法onConsumedSubpartition
	 *
	 *
	 */
